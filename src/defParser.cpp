...
int compf(defrCallbackType_e c, defiComponent* co, defiUserData ud) {
  if(testDebugPrint) {
    ...
    if(co->isFixed())
      fprintf(fout, "+ FIXED %d %d %s ", co->placementX(), co->placementY(),
              // orientStr(co->placementOrient()));
              co->placementOrientStr());
    if(co->isCover())
      fprintf(fout, "+ COVER %d %d %s ", co->placementX(), co->placementY(),
              orientStr(co->placementOrient()));
    if(co->isPlaced())
      fprintf(fout, "+ PLACED %d %d %s ", co->placementX(), co->placementY(),
              orientStr(co->placementOrient()));
    if(co->isUnplaced()) {
...
int netf(defrCallbackType_e c, defiNet* net, defiUserData ud) {
  ...
  for(i = 0; i < net->numVpins(); i++) {
    ...
    if(vpin->status() != ' ') {
      ...
      if(vpin->orient() != -1) fprintf(fout, " %s", orientStr(vpin->orient()));
    }
    fprintf(fout, "\n");
  }
  ...
  */
  ...
  fprintf(fout, ";\n");
  --numObjs;
  if(numObjs <= 0) fprintf(fout, "END NETS\n");
  return 0;
}
...
int snetpath(defrCallbackType_e c, defiNet* ppath, defiUserData ud) {
        ...
        while((path = (int)p->next()) != DEFIPATH_DONE) {
          ...
          switch(path) {
            ...
            case DEFIPATH_VIADATA:
              ...
              fprintf(fout, "DO %d BY %d STEP %d %d ", numX, numY, stepX,
                      stepY);
              break;
            case DEFIPATH_WIDTH:
              fprintf(fout, "%d ", p->getWidth());
  ...
  if(ppath->hasSpacingRules()) {
    for(i = 0; i < ppath->numSpacingRules(); i++) {
      ...
      if(left == right)
        ...
        fprintf(fout, "\n  + SPACING %s %g RANGE %g %g ", layerName, dist, left,
                right);
    }
  }
  ...
  if(ppath->hasVoltage()) fprintf(fout, "\n  + VOLTAGE %g ", ppath->voltage());
  ...
  if(ppath->hasPattern()) fprintf(fout, "\n  + PATTERN %s ", ppath->pattern());
...
int snetf(defrCallbackType_e c, defiNet* net, defiUserData ud) {
  ...
  if(net->numRectangles()) {
    for(i = 0; i < net->numRectangles(); i++) {
      ...
      if(net->rectMask(i)) {
        fprintf(fout, "\n  + MASK %d + RECT %s %d %d %d %d", net->rectMask(i),
                net->rectName(i), net->xl(i), net->yl(i), net->xh(i),
                net->yh(i));
      }
      else {
        fprintf(fout, "\n  + RECT %s %d %d %d %d", net->rectName(i), net->xl(i),
    ...
    for(i = 0; i < net->numViaSpecs(); i++) {
      ...
      if(net->topMaskNum(i) || net->cutMaskNum(i) || net->bottomMaskNum(i)) {
        fprintf(fout, "\n  + MASK %d%d%d + VIA %s ", net->topMaskNum(i),
                net->cutMaskNum(i), net->bottomMaskNum(i), net->viaName(i));
      }
      else {
        fprintf(fout, "\n  + VIA %s ", net->viaName(i));
      }
  ...
  if(net->hasOriginal()) fprintf(fout, "\n  + ORIGINAL %s ", net->original());
  if(net->hasUse()) fprintf(fout, "\n  + USE %s ", net->use());

  fprintf(fout, ";\n");
  --numObjs;
  if(numObjs <= 0) fprintf(fout, "END SPECIALNETS\n");
  return 0;
}
...
int ndr(defrCallbackType_e c, defiNonDefault* nd, defiUserData ud) {
  ...
  checkType(c);
  if(ud != userData) dataError();
  if(c != defrNonDefaultCbkType) fprintf(fout, "BOGUS NONDEFAULTRULE TYPE  ");
  fprintf(fout, "- %s\n", nd->name());
...
int cls(defrCallbackType_e c, void* cl, defiUserData ud) {
  ...
  switch(c) {
    ...
    case defrCannotOccupyCbkType:
      site = (defiSite*)cl;
      fprintf(fout, "CANNOTOCCUPY %s %g %g %s ", site->name(), site->x_orig(),
              site->y_orig(), orientStr(site->orient()));
      fprintf(fout, "DO %g BY %g STEP %g %g ;\n", site->x_num(), site->y_num(),
              site->x_step(), site->y_step());
      break;
      ...
      if(testDebugPrint) {
        ...
        if(pin->hasLayer()) {
          ...
          for(i = 0; i < pin->numLayer(); i++) {
            fprintf(fout, "\n  + LAYER %s ", pin->layer(i));
            if(pin->layerMask(i)) fprintf(fout, "MASK %d ", pin->layerMask(i));
          ...
          for(i = 0; i < pin->numVias(); i++) {
            if(pin->viaTopMask(i) || pin->viaCutMask(i) ||
              ...
              fprintf(fout, "\n  + VIA %s MASK %d%d%d %d %d ", pin->viaName(i),
                      pin->viaTopMask(i), pin->viaCutMask(i),
                      pin->viaBottomMask(i), pin->viaPtX(i), pin->viaPtY(i));
            }
            else {
        ...
        if(pin->hasPort()) {
          ...
          for(j = 0; j < pin->numPorts(); j++) {
            ...
            if(port->hasPlacement()) {
              if(port->isPlaced()) {
                fprintf(fout, "\n     + PLACED ");
                fprintf(fout, "( %d %d ) %s ", port->placementX(),
        ...
        if(pin->hasPlacement()) {
          if(pin->isPlaced()) {
            fprintf(fout, "+ PLACED ");
            fprintf(fout, "( %d %d ) %s ", pin->placementX(), pin->placementY(),
          ...
          if(pin->isUnplaced()) fprintf(fout, "+ UNPLACED ");
        }
        if(pin->hasSpecial()) {
        ...
        fprintf(fout, ";\n");
        --numObjs;
        if(numObjs <= 0) fprintf(fout, "END PINS\n");
      }
      break;
    ...
    case defrRowCbkType:
      ...
      fprintf(fout, "ROW %s %s %g %g %s ", ignoreRowNames ? "XXX" : row->name(),
              row->macro(), row->x(), row->y(), orientStr(row->orient()));
      if(row->hasDo()) {
    ...
    case defrTrackCbkType:
      ...
      if(track->firstTrackMask()) {
        ...
        fprintf(fout, "TRACKS %s %g DO %g STEP %g LAYER ", track->macro(),
                track->x(), track->xNum(), track->xStep());
      }
      ...
      if(testDebugPrint) {
        ...
        if(via->numPolygons()) {
          ...
          for(i = 0; i < via->numPolygons(); i++) {
            ...
            if(polyMask) {
              fprintf(fout, "\n  + POLYGON %s + MASK %d ", via->polygonName(i),
                      polyMask);
            }
            else {
      ...
      if(sc->hasOrdered()) {
        for(i = 0; i < sc->numOrderedLists(); i++) {
          ...
          for(j = 0; j < size; j++) {
            ...
            if(bits[j] != -1) fprintf(fout, "( BITS %d ) ", bits[j]);
            fprintf(fout, "\n");
          }
        }
      }
    ...
    case defrIOTimingCbkType:
      ...
      if(iot->hasDriveCell()) {
        ...
        if(iot->hasParallel()) fprintf(fout, "PARALLEL %g", iot->parallel());
        fprintf(fout, "\n");
      }
      fprintf(fout, ";\n");
      --numObjs;
    ...
    case defrSlotCbkType:
      slots = (defiSlot*)cl;
      if(slots->hasLayer()) fprintf(fout, "- LAYER %s\n", slots->layerName());
