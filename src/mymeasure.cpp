...
void CMeasure::accm_clock_start(string task) {
	...
	fatal(inputChk.find(task) != inputChk.end() && inputChk[task] == true, "Already started this task:"+task);
...
//	cout << task << ", start: " << double(time.tv_sec + .000001 * time.tv_usec) << endl;
...
void CMeasure::accm_clock_end(string task) {
	...
	timeStor[task] += double((time.tv_sec + .000001 * time.tv_usec) - timeTmpStor[task]);
//	cout << task << ", end: " << time.tv_sec + .000001* time.tv_usec << endl;
...
void CMeasure::accm_clock_print(string task) {
	...
	cout << " " << task << right << setw(30-task.size()) 
			<< fixed << setprecision(3) << timeStor[task] << endl;
}
...
//		int length1 = log10(celapsed_secs[i]) > 0? log10(celapsed_secs[i]): 0;
//		int length2 = log10(welapsed_secs[i]) > 0? log10(welapsed_secs[i]): 0;
//		int length = length1 + length2;

		printf(" %-25s %10.3f %10.3f\n", tasks[i].c_str(), welapsed_secs[i], celapsed_secs[i]);
